...
cmd /c "exit /b 0"
where python>nul
if not %errorlevel% equ 0 (goto end) else (
...
echo %LOGGING_PREFIX%在 %VENV_HOME% 激活了虚拟环境
...
rem useful 的长城互联网无法访问 huggingface.co 
...
if /i "%DO_DOWNLOAD_MODEL%"=="Y" (
    ...
    if not %errorlevel% equ 9009 (
        ...
        echo %LOGGING_PREFIX%检测到未安装 Git ，正在跳过...
    )
)

:main
echo %LOGGING_PREFIX%启动 Python 脚本
"%PYTHON_HOME%/python.exe" __main__.py
